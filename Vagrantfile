...
Vagrant.configure("2") do |config|
  ...
  # please see the online documentation at vagrantup.com.
    ...
    config.vm.define :entry1 do |entry1|
        entry1.vm.box = "entry1"
        entry1.vm.hostname = "entry1"
        #second is a private network
        entry1.vm.network :private_network, ip: "192.168.50.1"
        #we have a third interface for configuration with the server behind
        entry1.vm.network :private_network, ip: "192.168.55.1"

        entry1.vm.provision :puppet do |puppet|
    ...
    config.vm.define :entry2 do |entry2|
        entry2.vm.box = "entry2"
        entry2.vm.hostname = "entry2"
        #second is a private network
        entry2.vm.network :private_network, ip: "192.168.50.2"

        #we have a third interface for configuration with the server behind
        entry2.vm.network :private_network, ip: "192.168.55.2"

        entry2.vm.provision :puppet do |puppet|
            puppet.manifests_path = "manifests"
            puppet.manifest_file  = "entry.pp"
            puppet.module_path = "modules"
        end
   ...
   config.vm.define :server do |server|
        ...
        # private network
        server.vm.network :private_network, ip: "192.168.55.10"
  ...
  config.vm.box_url = "/home/formorer/vagrant/debian-7.1.0.box"
